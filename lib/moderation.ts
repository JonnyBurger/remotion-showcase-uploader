...
export async function getScores({
	...
	const files = timestamps.map(
		(time) => `https://image.mux.com/${playbackId}/thumbnail.png?time=${time}`
	);
	const annotations = await Promise.all(
		files.map((file) => fetchAnnotationsForUrl(file))
	);
	/*
	 * Filter out nulls to make typescript happy
	 */
	const annotationsFiltered = annotations.filter((a) =>
		Boolean(a)
	) as SafeSearchAnnotation[];
